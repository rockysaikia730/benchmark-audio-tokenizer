...
class BiCodec(nn.Module):
    ...
    def tokenize(self, batch: Dict[str, Any]):
        ...
        """
        semantic_tokens = self.get_semantic_tokens(batch)
        global_tokens = self.get_global_tokens(batch)

        return semantic_tokens, global_tokens
    
    @torch.no_grad()
    def get_semantic_tokens(self, batch: Dict[str, Any]):
        feat = batch["feat"]
        z = self.encoder(feat.transpose(1, 2))
    ...
    @torch.no_grad()
    def get_global_tokens(self, batch: Dict[str, Any]):
        mel = self.mel_transformer(batch["ref_wav"]).squeeze(1)
