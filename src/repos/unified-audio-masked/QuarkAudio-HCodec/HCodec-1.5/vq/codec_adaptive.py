...
class Codec(nn.Module):
    ...
    def forward(self, x, feat, use_mask=False, domain_split=None, padding_mask=None):
        ...
        # print('\n',x.shape,end='\n\n')  
        emb = self.encoder(x, padding_mask=padding_mask) # emb.shape [B, D, T]
        ...
        if self.use_similarity_alignment:
            ...
            alignment_matrices, sim, num_segments_per_item = FlexiCodec._perform_similarity_alignment_vectorized(
                semantic_emb.transpose(-2, -1),
                x_lens=x_lens,
        ...
        # deaggregate only undoes adaptive grouping; in fixed mode there's nothing to undo
        if self.use_similarity_alignment:
        ...
        # bottleneck_transformer is part of the adaptive recombination path; skip in fixed mode
        if self.use_bottleneck_transformer and self.use_similarity_alignment:
            cat_code_final = self.bottleneck_transformer(cat_code)
        else:
            cat_code_final = cat_code
        ...
        if self.use_similarity_alignment:
            ret_dict['token_lengths'] = alignment_matrices.sum(dim=2).long()

        return ret_dict

    @torch.no_grad()
    def encode(self, x, feat, use_mask=False, domain_split=None, threshold=0.0, padding_mask=None):
        ...
        acoustic_codes = acoustic_codes.transpose(-2, -1)                          # b,nq,t
